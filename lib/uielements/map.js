            ...
            if (self.webGLEnabled){
              ...
              Joshfire.require(['joshfire/vendor/webglearth'], function(){
                ...
                self.map = new WebGLEarth(self.htmlId, options);
                  document.getElementById(self.htmlId).className += " earth";
                self.map.setBaseMap(
          ...
          **/
            ...
            **/
            moveToLocationWebGL:function(location, options,callback){
                /** ,nothing here yet **/
                console.log('web gl, move to', location.city, location);

                // if we are at the same place we dont play the animation
                var current_position = this.map.getPosition();

                var lat = location.lat;
                var lng = location.lng;

                var computeDuration = function(current, next) {
                    // make the range in 0-360 degree
                    var current_lat = current[0];
                    var current_lng = current[1];

                    var next_lat = next[0];
                    var next_lng = next[1];

                    var delta_lat =  next_lat-current_lat;
                    var delta_lng =  next_lng-current_lng;
                    var dist = Math.sqrt(delta_lat*delta_lat + delta_lng*delta_lng);
                    
                    var ratio = (dist%180)/180.0;
                    
                    var minTime = 3;
                    var varyingTime = 4 * ratio;
                    var duration = varyingTime + minTime;
                    console.log("Distance = " + dist.toString() + " duration " + duration);
                    return duration;
                };

                var duration = computeDuration(current_position, [lat, lng ]) * 1000;
                var options3D = _.extend({}, this.getMap3DOptions(), options);

                var delta_lat = Math.abs(lat-current_position[0]);
                var delta_lng = Math.abs(lng-current_position[1]);
                console.log(location.city + " " + delta_lat.toString() + " " + delta_lng.toString());
                if (delta_lat < 1e-6 && 
                    delta_lng < 1e-6) {
                    console.log("same location, dont execute animation");
                    callback();
                    return;
                }

                this.map.flyTo(lat, lng, options3D.maxAscent, duration, options3D.altitude);
                // here we save the marker to remove it when the next animation will be finished
                var previous_marker = this.my_marker;
                this.my_marker = this.map.initMarker(lat, lng);
                setTimeout(function() {

                    if (previous_marker !== undefined) {
                        previous_marker.detach();
                    }
